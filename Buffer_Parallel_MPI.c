...
#include <time.h>
#include <stdbool.h>
#define AUTOMATIC_MATRIX_SIZE 8



//TRASPOSE FUNCTION
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void MatTranspose (float* local_transposed, float* local_buffer, int rows_per_proc, int N){
  for (int i = 0; i < rows_per_proc; i++) {
        for (int j = 0; j < N; j++) {
            local_transposed[j * rows_per_proc + i] = local_buffer[i * N + j];
        }
    }
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (rank == 0) { // The master process read the input
            ...
            while (inputVerification) {
                ...
                if ((N > 0)&&(N % size == 0)) { 
                    ...
                    printf("the size of the entered matrix is not a power of two or is not divisible by the number of processes entered \n");
                }
            }
        #endif
    }
...
//-------------------------------------------------------------------------
    ...
    float *local_transposed = malloc(N * rows_per_proc * sizeof(float));


    // compute transposed matrix in 1D representation
    MatTranspose(local_transposed, local_buffer, rows_per_proc, N);
    ...
    MPI_Gather(local_transposed, rows_per_proc * N, MPI_FLOAT, recv_buffer, rows_per_proc * N, MPI_FLOAT, 0, MPI_COMM_WORLD);
...
//----------------------------------------------------------------------------------------------------------------
    ...
    if (rank == 0) {
        ...
        for (int f = 0; f < size; f++) {
            for (int i = 0; i < N; i++) {
                for (int j = 0; j < rows_per_proc; j++) {
                    transposed[i][j + f * rows_per_proc] = recv_buffer[j + i * rows_per_proc + f * rows_per_proc * N];
                }
            }
        }
        ...
        //IF YOU WANT PRINT THE TRANSPOSED MATRIX REMOVE THE FOLLOWING COMMENTS
        /*
        printf("\nMatrice trasposta:\n");
